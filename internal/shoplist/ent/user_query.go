...
func (uq *UserQuery) QueryShopping() *ShoppingQuery {
	query := &ShoppingQuery{config: uq.config}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withShopping: uq.withShopping.Clone(),
...
func (uq *UserQuery) WithShopping(opts ...func(*ShoppingQuery)) *UserQuery {
	query := &ShoppingQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withShopping = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withShopping; query != nil {
		if err := uq.loadShopping(ctx, query, nodes,
			func(n *User) { n.Edges.Shopping = []*Shopping{} },
			func(n *User, e *Shopping) { n.Edges.Shopping = append(n.Edges.Shopping, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadShopping(ctx context.Context, query *ShoppingQuery, nodes []*User, init func(*User), assign func(*User, *Shopping)) error {
	...
	query.Where(predicate.Shopping(func(s *sql.Selector) {
		s.Where(sql.InValues(user.ShoppingColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_shopping
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_shopping" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_shopping" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
