...
func (iq *ItemQuery) QueryShopping() *ShoppingQuery {
	query := &ShoppingQuery{config: iq.config}
...
func (iq *ItemQuery) Clone() *ItemQuery {
	...
	return &ItemQuery{
		...
		withShopping: iq.withShopping.Clone(),
...
func (iq *ItemQuery) WithShopping(opts ...func(*ShoppingQuery)) *ItemQuery {
	query := &ShoppingQuery{config: iq.config}
	for _, opt := range opts {
		opt(query)
	}
	iq.withShopping = query
	return iq
}
...
func (iq *ItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Item, error) {
	...
	if query := iq.withShopping; query != nil {
		if err := iq.loadShopping(ctx, query, nodes, nil,
			func(n *Item, e *Shopping) { n.Edges.Shopping = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *ItemQuery) loadShopping(ctx context.Context, query *ShoppingQuery, nodes []*Item, init func(*Item), assign func(*Item, *Shopping)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Item)
	for i := range nodes {
		if nodes[i].shopping_item == nil {
			continue
		}
		fk := *nodes[i].shopping_item
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "shopping_item" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
