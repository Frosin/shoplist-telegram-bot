...
// Returns a *NotSingularError when more than one Shopping entity is found.
...
// Returns a *NotSingularError when more than one Shopping ID is found.
...
func (sq *ShoppingQuery) GroupBy(field string, fields ...string) *ShoppingGroupBy {
	grbuild := &ShoppingGroupBy{config: sq.config}
...
func (sq *ShoppingQuery) Select(fields ...string) *ShoppingSelect {
	...
	selbuild := &ShoppingSelect{ShoppingQuery: sq}
	selbuild.label = shopping.Label
...
func (sq *ShoppingQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Shopping, error) {
	var (
		nodes       = []*Shopping{}
		withFKs     = sq.withFKs
		_spec       = sq.querySpec()
	...
	_spec.ScanValues = func(columns []string) ([]interface{}, error) {
		return (*Shopping).scanValues(nil, columns)
	}
	...
	if query := sq.withItem; query != nil {
		if err := sq.loadItem(ctx, query, nodes,
			func(n *Shopping) { n.Edges.Item = []*Item{} },
			func(n *Shopping, e *Item) { n.Edges.Item = append(n.Edges.Item, e) }); err != nil {
			return nil, err
		}
	}
	if query := sq.withShop; query != nil {
		if err := sq.loadShop(ctx, query, nodes, nil,
			func(n *Shopping, e *Shop) { n.Edges.Shop = e }); err != nil {
			return nil, err
		}
	}
	if query := sq.withUser; query != nil {
		if err := sq.loadUser(ctx, query, nodes, nil,
			func(n *Shopping, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *ShoppingQuery) loadItem(ctx context.Context, query *ItemQuery, nodes []*Shopping, init func(*Shopping), assign func(*Shopping, *Item)) error {
	...
	for _, n := range neighbors {
		fk := n.shopping_item
		if fk == nil {
			return fmt.Errorf(`foreign-key "shopping_item" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "shopping_item" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (sq *ShoppingQuery) loadShop(ctx context.Context, query *ShopQuery, nodes []*Shopping, init func(*Shopping), assign func(*Shopping, *Shop)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Shopping)
	for i := range nodes {
		if nodes[i].shop_shopping == nil {
			continue
		}
		fk := *nodes[i].shop_shopping
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "shop_shopping" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (sq *ShoppingQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Shopping, init func(*Shopping), assign func(*Shopping, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Shopping)
	for i := range nodes {
		if nodes[i].user_shopping == nil {
			continue
		}
		fk := *nodes[i].user_shopping
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_shopping" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
